...
function ProductsView() {
  const { products, createProduct, updateProduct, toggleProductActive, deleteProduct, deleteGalleryImage } = useAdmin();
  ...
  const [createVals, setCreateVals] = useState({ name: '', volume_no: '', article_no: '', category: 'Ready to Wear', price: '', old_price: '', discount: '', stock: 10, image: null, sizes: ['M', 'L'], colors: [] });
  ...
  const startEdit = (p) => { 
    ...
    const cleanVal = (val) => (val === null || val === 'null' ? '' : val);
    ...
    setEditVals({ 
      ...
      sizes: p.sizes || [], 
      colors: p.colors || [],
      volume_no: cleanVal(p.volume_no),
      article_no: cleanVal(p.article_no)
    }); 
    console.log("Mapped state for editing:", {
      volume_no: cleanVal(p.volume_no),
      article_no: cleanVal(p.article_no)
    });
  ...
  const handleCreate = async () => {
    try {
      ...
      setShowCreate(false);
      setCreateVals({ name: '', volume_no: '', article_no: '', category: 'Ready to Wear', price: '', old_price: '', discount: '', stock: 10, image: null, gallery: [], sizes: ['M', 'L'], colors: [] });
    ...
    <div>
      ...
      {showCreate && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">Volume No</label>
              <input value={createVals.volume_no} onChange={e=>setCreateVals(v=>({...v,volume_no:e.target.value}))}
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">Article No (Auto-generated)</label>
              <input value={createVals.article_no} readOnly
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-gray-400 text-sm focus:outline-none cursor-not-allowed"
                placeholder="TW-123456 (Auto-generated)"/>
            </div>
            <div>
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">New Price (e.g. PKR 4,500)</label>
              <input value={createVals.price} onChange={e=>{
                const val = e.target.value;
                setCreateVals(v=>{
                  const newNum = parseFloat(val.replace(/[^0-9.]/g,''));
                  const oldNum = parseFloat((v.old_price||'').replace(/[^0-9.]/g,''));
                  const disc = (!isNaN(newNum) && !isNaN(oldNum) && oldNum > 0) ? Math.round((1 - newNum/oldNum)*100) : v.discount;
                  return {...v, price:val, discount: disc || v.discount};
                });
              }}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-gold/40 transition-all"
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">Old Price (e.g. PKR 6,000)</label>
              <input value={createVals.old_price} onChange={e=>{
                const val = e.target.value;
                setCreateVals(v=>{
                  const oldNum = parseFloat(val.replace(/[^0-9.]/g,''));
                  const newNum = parseFloat((v.price||'').replace(/[^0-9.]/g,''));
                  const disc = (!isNaN(newNum) && !isNaN(oldNum) && oldNum > 0) ? Math.round((1 - newNum/oldNum)*100) : v.discount;
                  return {...v, old_price:val, discount: disc || v.discount};
                });
              }}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-gold/40 transition-all"
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">Discount % Off (Auto or Manual)</label>
              <input type="number" min="0" max="100" value={createVals.discount} onChange={e=>setCreateVals(v=>({...v,discount:e.target.value}))}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-gold/40 transition-all"
                placeholder="e.g. 25"/>
            </div>
            <div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="border-b border-white/10">
              {['Image','Info','Category','Price','Stock','Status','Actions'].map(h=>(
                ...
                <tr key={p.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 max-w-[200px]">
                    <div className="flex flex-col">
                      ...
                      <div className="flex gap-2 mt-0.5">
                        {p.volume_no && <span className="text-[9px] text-gold font-black uppercase tracking-tighter bg-gold/5 px-1 rounded border border-gold/10">{p.volume_no}</span>}
                        {p.article_no && <span className="text-[9px] text-gray-500 font-bold uppercase tracking-tighter">#{p.article_no}</span>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-0.5">
                      {p.old_price && <span className="text-gray-500 text-xs line-through">{p.old_price}</span>}
                      <span className="text-gold text-sm font-semibold">{p.price}</span>
                      {p.discount && <span className="inline-block bg-red-500/20 text-red-400 text-[9px] font-black uppercase px-1.5 py-0.5 rounded-full border border-red-500/20 w-fit">{p.discount}% OFF</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gold mb-3">Volume No</label>
                    <input value={editVals.volume_no || ''} onChange={e=>setEditVals(v=>({...v,volume_no:e.target.value}))}
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gold mb-3">Article No (Auto-generated)</label>
                    <input value={editVals.article_no || ''} readOnly
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-gray-400 text-base focus:outline-none cursor-not-allowed"/>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gold mb-3">New Price</label>
                    <input value={editVals.price} onChange={e=>{
                      const val = e.target.value;
                      setEditVals(v=>{
                        const newNum = parseFloat(val.replace(/[^0-9.]/g,''));
                        const oldNum = parseFloat((v.old_price||'').replace(/[^0-9.]/g,''));
                  ...
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gold mb-3">Old Price</label>
                    <input value={editVals.old_price || ''} onChange={e=>{
                      const val = e.target.value;
                      setEditVals(v=>{
                        const oldNum = parseFloat(val.replace(/[^0-9.]/g,''));
                        const newNum = parseFloat((v.price||'').replace(/[^0-9.]/g,''));
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-gold mb-3">% Off (Auto / Manual)</label>
                    <div className="relative">
                      <input type="number" min="0" max="100" value={editVals.discount || ''} onChange={e=>setEditVals(v=>({...v,discount:e.target.value}))}
                        className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white text-base focus:outline-none focus:border-gold/50 transition-all"
                        placeholder="e.g. 25"/>
                      {editVals.discount && <span className="absolute right-4 top-1/2 -translate-y-1/2 bg-red-500/20 text-red-400 text-xs font-black px-2 py-0.5 rounded-full border border-red-500/30">{editVals.discount}% OFF</span>}
                    </div>
                  </div>
                  <div>
                    ...
                    {editGalleryPreviews.map((img, idx) => (
                      <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 group">
                        ...
                        <button type="button" 
                          onClick={async () => {
                            if (img.id) {
                              if (window.confirm("Delete image from server?")) {
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex gap-2">
                          <input placeholder="Color Name (optional)" value={customColorName} onChange={e => setCustomColorName(e.target.value)}
                            className="flex-1 bg-white/10 border border-white/5 rounded-xl px-4 py-3 text-white text-xs focus:outline-none focus:border-gold/40" />
                          <button type="button" onClick={() => {
                            const name = customColorName.trim() || customColorHex.toUpperCase();
                            const newColor = { name, hex: customColorHex };
                            if (!editVals.colors?.some(c => c.hex === customColorHex)) {
                              setEditVals(v => ({ ...v, colors: [...(v.colors || []), newColor] }));
                            }
                            setCustomColorName('');
                          }} className="gold-button px-4 rounded-xl text-[10px] font-black uppercase tracking-widest">
                            Add
                          </button>
                        </div>
                      </div>
...
function BannerView() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.bg_image === null) fd.append('bg_image', '');
      else if (form.bg_image) fd.append('bg_image', form.bg_image);
      
      if (form.left_image === null) fd.append('left_image', '');
      else if (form.left_image) fd.append('left_image', form.left_image);
      
      if (form.right_image === null) fd.append('right_image', '');
      else if (form.right_image) fd.append('right_image', form.right_image);
  ...
  const ImageUploadBox = ({ label, field, preview, currentUrl, setPreview }) => (
    <div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="relative w-20 h-24 rounded-xl overflow-hidden border border-white/10 flex-shrink-0 bg-white/5 group">
          {(preview || currentUrl) ? (
            <>
