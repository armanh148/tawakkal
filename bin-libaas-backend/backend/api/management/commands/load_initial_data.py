...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        # Create superuser
        if not User.objects.filter(username='admin@binlibaas.com').exists():
            User.objects.create_superuser('admin@binlibaas.com', 'admin@binlibaas.com', 'admin123')
            self.stdout.write(self.style.SUCCESS('Successfully created superuser'))

        # Load products
        products_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'binlibaas', 'products.json')
        try:
